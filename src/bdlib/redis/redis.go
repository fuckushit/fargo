...
import (
	...
	redis "github.com/garyburd/redigo/redis.v0"
)
...
// NewRedisManagerTimeout 新建redis管理器
func NewRedisManagerTimeout(host, auth string, poolsize int, conTimeout, rwTimeout time.Duration) (p *RedisManager, err error) {
	...
	p.auth = auth
	p.redisPool = make(chan redis.Conn, poolsize)
	p.conTimeout = conTimeout
	p.rwTimeout = rwTimeout
	for i := 0; i < poolsize; i++ {
		conn, err := p.dialRedisDiffTimeout()
		if err != nil {
			return nil, err
		}
		p.redisPool <- conn
...
func (w *RedisManager) dialRedisDiffTimeout() (conn redis.Conn, err error) {
	conn, err = redis.DialTimeout("tcp", w.host, w.conTimeout, w.rwTimeout, w.rwTimeout)
	if err != nil {
		return nil, err
	}
	if w.auth != "" {
...
func (w *RedisManager) GetSet(key string, value interface{}) (reply interface{}, err error) {
	action := func(conn redis.Conn) (interface{}, error) {
...
// // 运维的通用cache不支持此命令，如果谁用了，建议换Exists和SetEx组合使用
...
// IncrbyFloat float数据原子加
func (w *RedisManager) IncrbyFloat(key string, increment float64) (result float64, err error) {

	action := func(conn redis.Conn) (interface{}, error) {
		return conn.Do("INCRBYFLOAT", key, increment)
	}

	result, err = redis.Float64(w.Do(action))
	return
}
...
// Sunion 并集
func (w *RedisManager) Sunion(key ...string) (result []string, err error) {
	args := []interface{}{}
	for _, k := range key {
		args = append(args, k)
	}
